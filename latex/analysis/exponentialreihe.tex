...
\begin{theorem*}Satz 8.1.1
	...
	\begin{itemize}
		\item (A) $\exp(z+w)=\exp(z)\cdot \exp(w)$
		\item (W) $\lim \limits_{z \to\infty}\frac{\exp(z)-1}{z}=1$
	\end{itemize}
	Für diese Funktion gilt:
	\begin{itemize}
		\item (i) $\exp(z)=\sum_{n=0}^{\infty}\frac{z^{n}}{n!}$
		\item (ii) $\exp(z)=\lim \limits_{n \to\infty}(1+\frac{z}{n})^{n}$
		\item $\exp(z)$ ist stetig und $e^{a}=\exp(q), \forall q\in \mathbb{Q}$, wobei $e=\exp(1)$ ist die Eulersche Zahl
	\end{itemize}
\end{theorem*}
